// // File: src/components/barDetails/ReservationDialog.tsx
...
// src/components/barDetails/ReservationDialog.tsx
import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuth0 } from '@auth0/auth0-react';
import {
...
const ReservationDialog: React.FC<Props> = ({ barId, barName, openingHours }) => {
  const navigate = useNavigate();
  const { isAuthenticated, loginWithRedirect } = useAuth0();
  const [people, setPeople] = useState('2');
  const today = new Date().toISOString().split('T')[0];
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button size="lg" className="absolute bottom-4 right-4 z-50 bg-amber-600 hover:bg-amber-500">
          Reserve a Table
        </Button>
      </DialogTrigger>
