...
//                                   variant="outline"
//                                   className="flex-1"
//                                   onClick={() => navigate(`/admin/bar-owners/:id/edit`)}
//                                 >
//                                   Edit
//                                 </Button>
//                                 <Button
...
import { useParams } from "react-router-dom";
import { useGetBarOwnerById, useGetBarOwnerBars, useDeleteBar } from "@/api/AdminApi";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { useNavigate } from "react-router-dom";
import { Bar } from "@/types";
...
const AdminBarOwnerBars = () => {
  ...
  const { barOwner, isLoading: isLoadingOwner } = useGetBarOwnerById(id!);
  const { bars, isLoading: isLoadingBars } = useGetBarOwnerBars(id!);
  const { deleteBar, isLoading: isDeleting } = useDeleteBar();
  const navigate = useNavigate();
              ...
              <CardContent className="p-4">
                ...
                <div className="mt-4 space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Button 
                      variant="outline"
                      onClick={() => navigate(`/admin/bars/${bar._id}/edit`)}
                      className="flex-1"
                    >
                      Edit
                    </Button>
                    <Button 
                      variant="destructive"
                      onClick={() => handleDeleteBar(bar._id)}
                      disabled={isDeleting}
                      className="flex-1"
                    >
                      Delete
